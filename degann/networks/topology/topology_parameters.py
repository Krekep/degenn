...
class TensorflowDenseNetParams(BaseTopologyParams):
    ...
    weight: Any = field(default_factory=lambda: tf.random_uniform_initializer(minval=-1, maxval=1))
    biases: Any = field(default_factory=lambda: tf.random_uniform_initializer(minval=-1, maxval=1))

    def __post_init__(self):
        self.net_type = "DenseNet"

@dataclass(kw_only=True)  # kw_only used to bypass the attribute organisation of @dataclass
class GANTopologyParams(BaseTopologyParams):
    ...
    def __post_init__(self):
        ...
        self.block_size = (self.generator_params.block_size +
